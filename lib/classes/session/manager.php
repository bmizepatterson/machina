...
class manager {
    ...
    public static function start() {
        ...
        if (empty($DB) or !defined('NO_MACHINA_COOKIES') or NO_MACHINA_COOKIES) {
        ...
        try {
            ...
            if (!self::$handler->start()) {
                ...
                throw new \session\exception('Alas, an error occurred whilst communicating with the server.');
            }
    ...
    public static function get_performance_info() {
        ...
        $info = array();
        $info['size'] = $size;
        $info['html'] = "<p>Session ($handler): $size</p>";
    ...
    protected static function prepare_cookies() {
        ...
        $sessionname = 'MachinaSession'.$CFG->sessioncookie;
    ...
    protected static function initialise_user_session($newsid) {
        ...
        if (!$record = $DB->get_record('SELECT id, sid, state, userid, lastip, timecreated, timemodified 
                                        FROM sessions WHERE sid = ?', array($sid))) {
            if (!$newsid) {
        ...
        if (isset($_SESSION['USER']->id)) {
            ...
            if ($timeout) {
                session_regenerate_id(true);
                $_SESSION = array();
                $DB->delete_records('DELETE FROM sessions WHERE id = ?', array($record->id));
                ...
                if ($updated) {
                    ...
                    $DB->update_record($query, $queryparams);
                }
    ...
    protected static function add_session_record($userid) {
        ...
        $insert = "INSERT INTO sessions (state, sid, sessdata, userid, timecreated, timemodified, firstip, lastip)
                   VALUES (:state, :sid, :sessdata, :userid, :timecreated, :timemodified, :firstip, :lastip)";
        $params = array();
        $params[':state']       = 0;
        $params[':sid']         = session_id();
        $params[':sessdata']    = null;
        $params[':userid']      = $userid;
        $params[':timecreated'] = $params[':timemodified'] = time();
        $params[':firstip']     = $params[':lastip'] = getremoteaddr();
        
        $record = $DB->insert_record($insert, $params);
    ...
    protected static function check_security() {
        ...
        if (!empty($_SESSION['USER']->id) and !empty($CFG->tracksessionip)) {
            ...
            if ($_SESSION['USER']->sessionip != $remoteaddr) {
                ...
                $description = '<p>Sorry, but your IP address seems to have changed from when you first logged in. 
                                   This security feature prevents crackers stealing your identity while logged in to this site. 
                                   You may see this error if you use wireless networks or if you are roaming between different 
                                   networks. Please ask the site administrator for more help.</p>
                                <p>If you want to continue please press the F5 key to refresh this page.</p>';
                throw new exception('IP Address Issue', $description);
            }
        }
    }
    ...
    public static function login_user(\stdClass $user) {
        ...
        $sid = session_id();
        session_regenerate_id(true);
        $DB->delete_record('DELETE FROM sessions WHERE sid = :sid', array(':sid'=>$sid));
    ...
    public static function session_exists($sid) {
        ...
        if (!$record = $DB->get_record('SELECT id, userid, timemodified FROM sessions WHERE sid = ?', array($sid))) {
            return false;
        }
    ...
    public static function touch_session($sid) {
        ...
        $DB->update_record($sql, array(':now'=>time(), ':sid'=>$sid));
    }
    ...
    public static function kill_all_sessions() {
        ...
        try {
            $DB->delete_record('DELETE FROM sessions');
        } catch (\database_delete_exception $ignored) {
    ...
    public static function kill_user_sessions($userid, $keepsid = null) {
        ...
        $sessions = $DB->get_records('SELECT id, sid FROM sessions WHERE userid = :userid ORDER BY id DESC', array(':userid'=>$userid));
    ...
    public static function apply_concurrent_login_limit($userid, $sid = null) {
        ...
        $count = $DB->get_records('SELECT COUNT(*) FROM sessions WHERE userid = :userid', array(':userid' => $userid));
